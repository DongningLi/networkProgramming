...
public class ModifyCommand {
	...
	public static ArrayList<PeerCon> getPeersList(List list) {
		ArrayList<PeerCon> peersInfoLs = new ArrayList<PeerCon>();
		int peersNumber = list.size();
		Iterator it = list.iterator();
		while (it.hasNext()) {
			Map hm = (Map) it.next();
			String name = hm.get("name") + "";
			int portNumber = Integer.parseInt(hm.get("portNumber") + "");
			String ipAdress = hm.get("IPAddress") + "";
			PeerCon pc = new PeerCon(name, portNumber, ipAdress);
			peersInfoLs.add(pc);
		}
		return peersInfoLs;
	}
	...
	public String getLocalTime(){
		...
		Date now = new Date();
		String LOCAL_TIME = simpleDateFormat.format(calendar.getTime());
