...
public class ASN1_Util {
	...
	public static Calendar getCalendar(String gdate, Calendar def) {
		...
		try {
			date.set(Integer.parseInt(gdate.substring(0, 4)), Integer.parseInt(gdate.substring(5, 7)) - 1,
					Integer.parseInt(gdate.substring(8, 10)), Integer.parseInt(gdate.substring(11, 13)),
					Integer.parseInt(gdate.substring(14, 16)), Integer.parseInt(gdate.substring(17, 19)));
			date.set(Calendar.MILLISECOND, Integer.parseInt(gdate.substring(20, 23)));
		} catch (Exception e) {
			return def;
		}
	...
	/**
	 * This is based on bit shifting (using base128()). Returns array of bytes.
	 * 
	 * @param val
	 * @return
	 */
	...
	/**
	 * Decodes from base 128 under the assumption that it is bigendian,
	 * terminated by a byte smaller than 128, all other bytes being OR-ed with
	 * 128.
	 * 
	 * @param b128
	 * @param offset
	...
	public static BigInteger fromBase128(byte[] b128, int offset, int limit) {
		...
		while ((k < limit) && ((b128[k] & 0x80) != 0)) {
			// result = result.multiply(Util.BN128).add(new BigInteger("" +
			// (Util.getUnsignedShort(b128[k]) - 128)));
		...
		if (k < limit) {
			// result = result.multiply(Util.BN128).add(new BigInteger("" +
			// Util.getUnsignedShort(b128[k])));
			if ((b128[k] & 0x80) != 0) {
				if (_DEBUG)
					System.out.println("Util: fromBase_128: last byte > 127");
				ASN1_Util.printCallPath("");
			}
			result = result.shiftLeft(7).or(new BigInteger("" + (b128[k]))); // here
																				// &
																				// 0x7f
																				// would
																				// be
