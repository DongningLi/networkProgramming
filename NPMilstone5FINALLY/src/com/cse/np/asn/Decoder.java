...
public class Decoder {
	...
	int offset;
	/**
	 * The length of the current component being decoded, after which null is
	 * returned even if "data" has more data
	 */
	int length;
	...
	public Decoder(byte[] data) {
		if (data == null)
			throw new RuntimeException("parameter data should not be null");
	...
	/**
	 * Returns the class value, which can be one of: Encoder.CLASS_APPLICATION,
	 * Encoder.CLASS_CONTEXT, Encoder.CLASS_PRIVATE, Encoder.CLASS_UNIVERSAL
	 * 
	 * @return
	 */
	 ...
	 * Encoder.PC_CONSTRUCTED, Encoder.PC_PRIMITIVE
	 * 
	 * @return
	 */
	public int typePC() {
	...
	public int typeLen() {
		...
		int k = offset + 1;
		int len = 2;
		while ((k < length + offset) && (ASN1_Util.getUnsignedShort(data[k]) > 127)) {
			len++;
			k++;
		}
		if (len > length)
	...
	public int contentLength() {
		...
		if (ASN1_Util.getUnsignedShort(data[offset + tlen]) < 128) {
			// System.err.println("in ContentLength:
			// len_len=="+data[offset+tlen]+" off="+offset);
			return data[offset + tlen];
		}
		// System.err.println("Len_len="+data[offset+tlen]+"
		// gs="+gs(data[offset+tlen]));
	...
	/**
	 * 
	 * @return -1 : not enough bytes in buffer to determine type and length of
	 *         length -k: not enough bytes (k-1 needed) to determine length of
	 *         length larger than 0: returns length of the buffer needed to
	 *         store the whole first ASN1 object
	 * 
	 */
	public int objectLen() {
		int type_len = typeLen();
		if ((type_len == 0) || (type_len >= length))
			return -1; // insufficient to find
		int len_len = lenLen();
		if ((len_len == 0) || (len_len + type_len > length))
			return -(type_len + len_len + 1);
	 ...
	 * @param type
	 * @param alist
	 *            : from ArrayList? If alist is set, then each element is
	 *            translated in an entry where the value is a generalized time
	 *            computed from the position. Otherwise it extracts a Hashtable,
	 *            each element from a different SEQUENCE
	 * @return
	...
	public Hashtable<String, String> getSequenceOfHSS(byte type, boolean alist) throws ASN1DecoderFail {
		...
		for (;;) {
			...
			if (!alist) {
				...
				if (d_t != null)
					val = d_t.getString(type);
			}
			if (val == null) {
				if (DEBUG_TODO)
					System.out.println("Decoder: transient versions!");
	...
	public ArrayList<String> getSequenceOfAL(byte type) throws ASN1DecoderFail {
		if (getTypeByte() == Encoder.TAG_NULL)
			return null;
		Decoder dec = getContent();
	...
	public <T> ArrayList<T> getSequenceOfAL(byte type, T inst) throws ASN1DecoderFail {
		...
		for (;; k++) {
			...
			try {
				...
				if (DEBUG)
					System.out.println("Decoder:getSeqOfAl:k=" + k);
				e.printStackTrace();
			}
			if (d_t == null)
				break;
	 ...
	 * @param <T>
	 * @param type
	 *            the ASN type of each element in the sequence
	 * @param temp
	 *            a empty array prototype
	 * @param inst
	 ...
	 * 
	 * @param extract
	 *            Changes this to get rid of the first Object
	...
	public Decoder getFirstObject(boolean extract) {
		...
		// lens="+tlen+"+"+llen+"+"+cLen+"<="+length +" off="+offset);
		int new_len = tlen + llen + cLen;
		if (new_len > length || new_len < 0)
			throw new ASNLenRuntimeException(
					"ASN1:Decoding:Invalid object length [" + new_len + "]: Too long given available data:" + length);
		int old_offset = offset;
		if (extract) {
			offset += new_len;
			length -= new_len;
			// System.out.println("Decoder: remains
			// lens="+tlen+"+"+llen+"+"+cLen+"<="+length +" off="+offset);
			if (offset < 0 || length < 0)
				throw new ASNLenRuntimeException(
						"Arrive at negative offset after extracting " + new_len + "/" + length);
		}
		// System.out.println("Decoder: extracts lens="+new_len+"<="+length +"
		// off="+old_offset+"/"+data.length);
	...
	public Decoder getFirstObject(boolean extract, byte type) throws ASN1DecoderFail {
		...
		if (new_len > length || new_len < 0)
			throw new ASNLenRuntimeException("Too long");
		int old_offset = offset;
		if (extract) {
			offset += new_len;
			length -= new_len;
	...
	public Decoder getContent() throws ASN1DecoderFail {
		...
		if (length <= 0)
			throw new ASNLenRuntimeException("Container length 0");
		int new_len;
		...
		/*
		 * if(new_off>length) throw new ASN1DecoderFail("Content "
		 * +new_off+":"+new_len+" exceeds container "+length); if(new_off<0)
		 * throw new ASN1DecoderFail("Content "+new_off+":"+new_len+
		 * " has negative offset in container length "+length);
		 * if(new_off+new_len>length) throw new ASN1DecoderFail("Content "
		 * +new_off+":"+new_len+" exceeds container "+length);
		 * if(new_len>length) throw new ASN1DecoderFail("Too long"); if(new_len
		 * < 0) throw new ASN1DecoderFail("Negative length");
		 */
	...
	public boolean getBoolean() {
		if (length <= 0)
			throw new ASNLenRuntimeException("Boolean length");
		// assert
	...
	public boolean getBoolean(byte type) throws ASN1DecoderFail {
		if (length <= 0)
			throw new ASNLenRuntimeException("Boolean length");
		if (data[offset] != type)
			throw new ASN1DecoderFail("Wrong boolean type");
	...
	public BigInteger getInteger() {
		...
		try {
			if (DEBUG)
				System.out.println("Decoder: getInteger: type=" + data[offset] + " vs=" + Encoder.TAG_INTEGER);
			// assert
		...
		if (DEBUG)
			System.out.println(
					"Decoder: getInteger: result = " + ASN1_Util.byteToHex(data, offset, typeLen() + lenLen(), " "));
		...
		if (DEBUG)
			System.out.println("Decoder: getInteger: result = " + result);
		return result;
	}
	...
	public BigInteger getInteger(byte tagInteger) throws ASN1DecoderFail {
		...
		if (data[offset] != tagInteger)
			throw new ASN1DecoderFail("Wrong tag");
		byte[] value = new byte[contentLength()];
		Encoder.copyBytes(value, 0, data, contentLength(), offset + typeLen() + lenLen());
	...
	public byte[] getBytes() {
		// assert
		if (!((data[offset] == Encoder.TAG_OCTET_STRING) || (data[offset] == Encoder.TAG_BIT_STRING)
	...
	/**
	 * The number of padding bits is stored in the parameter if this has at
	 * least one byte;
	 * 
	 * @param bits_padding
	 * @return
	...
	public byte[] getBytes(byte type) throws ASN1DecoderFail {
		if (getTypeByte() == Encoder.TAG_NULL)
			return null;
		if (data[offset] != type)
			throw new ASN1DecoderFail("OCTET STR: " + type + " != " + data[offset]);
		byte[] value = new byte[contentLength()];
		Encoder.copyBytes(value, 0, data, contentLength(), offset + typeLen() + lenLen());
		return value;
	}
	...
	public int[] getOID(byte type) throws ASN1DecoderFail {
		...
		int len = 2;
		for (int k = 1; k < b_value.length; k++)
			if (b_value[k] >= 0)
				len++;
	...
	public BigInteger[] getBNOID(byte type) throws ASN1DecoderFail {
		...
		for (int k = 2, crt = 1; k < value.length; k++) {
			// value[k] = ASN1_Util.fromBase128(b_value, k, this.offset +
			// this.length);
	...
	public String getString() {
		...
		// assert
		if (!((data[offset] == Encoder.TAG_IA5String) || (data[offset] == Encoder.TAG_UTF8String)
	...
	public String getString(byte type) throws ASN1DecoderFail {
		if (getTypeByte() == Encoder.TAG_NULL)
			return null;
		if (data[offset] != type)
			throw new ASN1DecoderFail("String: exp=" + type + " != in=" + data[offset]);
		byte[] value = new byte[contentLength()];
		Encoder.copyBytes(value, 0, data, contentLength(), offset + typeLen() + lenLen());
		try {
			return new String(value, "UTF-8");
	...
	public String getGeneralizedTime(byte type) throws ASN1DecoderFail {
		if (getTypeByte() == Encoder.TAG_NULL)
			return null;
		if (data[offset] != type)
			throw new ASN1DecoderFail("generalizedTime: " + type + " != " + data[offset]);
	...
	public boolean fetchAll(InputStream is) throws IOException {
		...
		while (true) {
			...
			if (DEBUG)
				System.out
						.println("Object size=" + asrlen + "; Buffer size=" + data.length + "; Current size=" + length);
			...
			if ((asrlen < 0) || (length < asrlen)) {
				if (DEBUG)
					System.out.println("Object size=" + asrlen + "; Current size=" + length);
	...
	public int[] getIntsArray() {
		if (this.getTypeByte() == Encoder.TAG_NULL)
			return null;
		Decoder dec;
		try {
			dec = this.getContent();
	...
	/**
	 * compares the parameter tag with the result of getTypeByte() when there is
	 * a next object (peeked)
	 * 
	 * @param tag
	 * @return
	 */
	...
	public static void encodeDecodeBNTAG(String nb) throws ASN1DecoderFail {
		...
		msg = enc.getBytes();
		if (DEBUG)
			System.out.println("BN: " + ASN1_Util.byteToHex(msg));
