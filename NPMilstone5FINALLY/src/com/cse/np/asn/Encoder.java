...
public class Encoder {
	...
	/*
	 * public static Calendar getCalendar(String gdate) { return
	 * Util.getCalendar(gdate); }
	 */
	...
	BigInteger contentLength() {
		if (header_length.length == 0)
			return BigInteger.ZERO;
		if (header_length[0] >= 0)
			return new BigInteger("" + header_length[0]);
	...
	/**
	 * Sets length in header_length (which is array of bytes). If length <= 127,
	 * then use a single byte. Else, one byte with (128+length) and the bytes
	 * needed to represent length (and sign).
	 * 
	...
	void setASN1Length(int _bytes_len) {
		...
		// System.err.println("set bytes="+bytes+"+"+header_length.length +"-"+
		// old_len_len);
	...
	/**
	 * sets the byte is header_type (updating bytes under assumption that the
	 * type was length was precomputed)
	 * 
	 * @param tagASN1
	 * @return
	 */
	 ...
	 * 
	 * @param classASN1
	 *            The ASN1 Class (UNIVERSAL/APPLICATION/CONTEXT/PRIVATE)
	 * @param PCASN1
	 *            Is this PRIMITIVE or CONSTRUCTED
	 * @param tag_number
	 *            : at most 30
	 * @return returns this
	 */
	public static byte buildASN1byteType(int classASN1, int PCASN1, byte tag_number) {
		if ((tag_number) >= 31) { // tag_number&0x1F
			if (tag_number != Encoder.TAG_SEQUENCE)
				ASN1_Util.printCallPath("Need more bytes for:" + tag_number);
	...
	/**
	 * result can be Encoder.CLASS_UNIVERSAL, Encoder.CLASS_APPLICATION,
	 * Encoder.CLASS_CONTEXT or Encoder.CLASS_PRIVATE
	 * 
	 * @param type
	 * @return
	 */
	...
	public Encoder setASN1Type(int classASN1, int PCASN1, BigInteger tag_number) {
		if (DEBUG)
			System.out.println("Encoder: setASN1Type: BN" + tag_number + " bytes=" + bytes);
		...
		if (DEBUG)
			System.out.println("Encoder: setASN1Type: BN old_len_len=" + old_header_type_len);
		int tag = (classASN1 << 6) + (PCASN1 << 5) + 0x1f;
		if (DEBUG)
			System.out.println("Encoder: setASN1Type: BN tag=" + tag);
		byte[] nb = ASN1_Util.toBase_128(tag_number); // String nb =
														// tag_number.toString(128);
		if (DEBUG)
			System.out.println("Encoder: setASN1Type: BN tag_128=" + ASN1_Util.byteToHex(nb));
		int tag_len = nb.length;
		if (DEBUG)
			System.out.println("Encoder: setASN1Type: BN tag_len=" + tag_len);
		header_type = new byte[tag_len + 1];
		header_type[0] = (byte) tag;
		for (int k = 0; k < tag_len - 1; k++) {
			header_type[k + 1] = (byte) (nb[k] | 0x80);
			if (DEBUG)
				System.out.println("Encoder: setASN1Type: BN tag_len=" + header_type[k + 1]);
		}
		header_type[tag_len] = (byte) nb[tag_len - 1];
		if (DEBUG)
			System.out.println("Encoder: setASN1Type: BN tag_len=" + header_type[tag_len - 1]);
	 ...
	 * 
	 * Sets this to the top of the tree, and delegates previous content to the
	 * "prefix_data", updating "bytes"
	 * 
	 * @param asn1_data
	 * @param offset
	 * @param length
	...
	public static String getGeneralizedTime(Calendar time) {
		...
		// System.out.println("getGeneralizedTime............. "+result+" from
		// "+time);
		return result;
	}
	...
	public Encoder(Calendar time) {
		...
		/*
		 * String
		 * UTC=String.format("%1$tY%1$tm%1$td%1$tH%1$tM%1$tS.%1$tLZ",time);
	...
	public Encoder(int[] oid) {
		...
		if (len < 0) {
			if (DEBUG)
				ASN1_Util.printCallPath("Wrong oid length: min 2!");
			return;
		}
		...
		for (int k = 2; k < oid.length; k++) {
			...
			for (int j = 0; j < b.length; j++, coff++)
				data[coff] = (byte) (b[j] | (byte) ((j < b.length - 1) ? 0x80 : 0));
		}
	...
	public Encoder(BigInteger[] oid) {
		...
		for (int k = 2; k < oid.length; k++)
			result.addAll(ASN1_Util.base128(oid[k]));
		data = new byte[result.size()];
		for (int k = 0; k < data.length; k++)
			data[k] = result.get(k).byteValue();
		setASN1Type(Encoder.TAG_OID);
		setASN1Length(data.length);
		bytes += data.length;
		assert (bytes == this.header_type.length + this.header_length.length + data.length);
	...
	public Encoder(String s) {
		...
		if (DEBUG)
			System.out.println("Encoder<init,String>: bytes=" + bytes + "+" + data.length);
		setASN1Length(data.length);
		if (DEBUG)
			System.out.println("Encoder<init,String>: bytes=" + bytes + "+" + data.length);
		bytes += data.length;
	...
	/**
	 * Returns NULLOCTETSTRING, i.e., NULL ASN object for a null parameter, and
	 * OCTETSTRING otherwise
	 * 
	 * @param s
	 */
	 ...
	 * 
	 * @param s
	 * @param ascii_vs_printable
	 *            : if true IA5String else PrintableString
	 */
	public Encoder(String s, boolean ascii_vs_printable) {
		...
		if (ascii_vs_printable)
			setASN1Type(Encoder.TAG_IA5String);
		else
	 ...
	 * 
	 * @param param,
	 *            an array of String[] to encode
	 * @param type,
	 *            the type to assign to each String @return, an Encoder
	 */
	public static Encoder getStringEncoder(String[] param, byte type) {
		...
		for (int k = 0; k < param.length; k++) {
			if (param[k] != null)
				...
				enc.addToSequence(Encoder.getNullEncoder());
		}
		return enc;
	}
	...
	public static Encoder getStringsEncoder(ArrayList<String> param, byte type) {
		...
		for (int k = 0; k < param.size(); k++) {
			...
			if (crt != null)
				enc.addToSequence(new Encoder(crt, type));
			else
				enc.addToSequence(Encoder.getNullEncoder());
		}
		return enc;
	...
	/**
	 * Uses the type for each element that is not null. uses TAG_NULL for null
	 * elements.
	 * 
	 * @param param
	 * @param type
	...
	/**
	 * Uses the type TAG_INTEGER for each element that is not null. uses
	 * TAG_NULL for null elements.
	 * 
	 * @param param
	 * @return
	 */
	...
	public static <T> Encoder getEncoder(ArrayList<T> param) {
		...
		for (int k = 0; k < param.size(); k++) {
			if (param.get(k) != null)
				enc.addToSequence(((ASNObj) param.get(k)).getEncoder());
			else
				enc.addToSequence(Encoder.getNullEncoder());
		}
		return enc;
	...
	public static <T> Encoder getEncoder(T[] param, ArrayList<String> dictionary_GIDs) {
		...
		for (int k = 0; k < param.length; k++) {
			if (param[k] != null)
				enc.addToSequence(((ASNObj) param[k]).getEncoder(dictionary_GIDs));
			else
				enc.addToSequence(Encoder.getNullEncoder());
		}
		return enc;
	...
	public static <T> Encoder getEncoder(T[] param, ArrayList<String> dictionary_GIDs, int dependants) {
		...
		for (int k = 0; k < param.length; k++) {
			if (param[k] != null)
				enc.addToSequence(((ASNObj) param[k]).getEncoder(dictionary_GIDs, dependants));
			else
				enc.addToSequence(Encoder.getNullEncoder());
		}
		return enc;
	...
	/**
	 * places the keys of param at indexes specified by
	 * Util.getCalendar(value).getTimeInMillis() -
	 * Encoder.CALENDAR_DISPLACEMENT; Adds NULL for null values and sets type to
	 * other elements
	 * 
	 * @param _param
	 * @param type
	...
	private static String[] convertHashtableToArray(Hashtable<String, String> _param) {
		if (_param == null)
			return null;
	...
	/**
	 * Uses sorted keys of param to return HSS_Elem ::=SEQUENCE {key [type]
	 * IMPLICIT UTF8String, value [type] IMPLICIT UTF8String OPTIONAL} SEQUENCE
	 * OF HSS_Elem
	 * 
	 * @param param
	 * @param type
	 * @return
	 */
	public static Encoder getHashStringEncoder(Hashtable<String, String> param, byte type) {
		if (param == null) {
			return Encoder.getNullEncoder();
		}
		...
		for (String s : keys) {
			...
			if (val != null)
				e.addToSequence(new Encoder(val, type));
			enc.addToSequence(e);
		}
		return enc;
	}
	...
	public static void main(String args[]) throws ASN1DecoderFail {
		Encoder seq2 = (new Encoder()).initSequence().addToSequence(new Encoder((byte) 3))
		...
		Encoder my_seq = (new Encoder()).initSequence().addToSequence(my_int).addToSequence(my_int2)
				.addToSequence(seq2);
